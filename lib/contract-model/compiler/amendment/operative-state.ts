...
function resolveBaseText(group: ProvisionGroup, baseDocumentId: string, index: StructuralIndex): BaseTextResolution {
  ...
  // already applies via resolveUniqueNodeByRef's own status. This is what
  // makes the fix robust even against the separate cross-document
  // fallback on the very next line (owned by another workstream, left
  // untouched here): that fallback may still supply a `def`/text value,
  // but it is never trusted as evidence of uniqueness on its own.

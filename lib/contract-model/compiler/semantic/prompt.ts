...
export function buildSystemPrompt(opts: { irSchemaVersion: string; toolPolicyVersion: string }): string {
  return [
    ...
    "",
    "SHARED CAPS AND CROSS-UNIT DEPENDENCIES (a frequent real omission): when two or more independently operative permissions draw on ONE aggregate ceiling ('together with amounts under clause (x) shall not exceed...', 'in the aggregate with...', 'without duplication of amounts used under...'), the ceiling is a SHARED resource, never a separate independent cap restated on each member. Emit exactly one WireSharedCapacity carrying the ceiling expression, list every member rule in memberRefs, and reference it from each member's capacityExpression via LEDGER_USAGE_REFERENCE (sharedCapRef) or by making the member's own cap SUBTRACT that usage - do NOT emit N freestanding rules each asserting the full ceiling as if it were independent (that silently multiplies capacity). When a sharing partner or a referenced provision lives in ANOTHER section that is not part of this submission, still emit the relationship as a dependsOn whose targetRef is the exact section reference (e.g. 'Section 6.01(b)(iii)') with relationshipType SHARES_CAPACITY_WITH/LIMITED_BY/REQUIRES as appropriate - it is preserved as an explicit unresolved cross-unit dependency for review, never dropped and never guessed. Use getReferencedProvision to read the partner provision when you need its economics.",
    "",
    "ENTITY SCOPE (a real, systematic omission): every rule carries `entityScope` and `entityScopeExcluded` - arrays of entity-class tags naming WHO the rule binds or who may rely on it (the borrower, a restricted or unrestricted subsidiary, a loan party, etc., using the same tag vocabulary as ENTITY_SCOPE_REFERENCE). Fill them from the provision's own words ('the Borrower shall not, and shall not permit any Restricted Subsidiary to', 'Indebtedness of any Loan Party', 'other than an Unrestricted Subsidiary'). Leave them empty ONLY when the provision genuinely names no entity class.",
    "",
...
export function buildFewShotExamplesBlock(): string {
  const examples = [
    ...
    {
      title: "Shared aggregate cap across two sibling permissions - ONE WireSharedCapacity, referenced from each member, never two independent caps",
      sourceText: "§9.06: (a) the Company may make Zeta Loans, and (b) the Company may issue Zeta Guarantees; provided that the aggregate amount of Zeta Loans outstanding under clause (a) together with the aggregate amount of Zeta Guarantees outstanding under clause (b) shall not at any time exceed 12% of Zeta Base Amount; provided further that amounts outstanding under Section 9.08(d) shall be included in such aggregate.",
      wireRules: [
        { localRef: "r8a", sourceSectionRef: "9.06(a)", covenantFamily: "INDEBTEDNESS", ruleType: "QUANTITATIVE_PERMISSION", posture: "PERMISSION", action: "INCUR_DEBT", capacityExpression: { kind: "LEDGER_USAGE_REFERENCE", sharedCapRef: "sc1", citation: "§9.06(a)" }, dependsOn: [{ relationshipType: "SHARES_CAPACITY_WITH", targetRef: "r8b", description: "shares the 12% aggregate ceiling with clause (b)" }, { relationshipType: "SHARES_CAPACITY_WITH", targetRef: "Section 9.08(d)", description: "amounts under Section 9.08(d) count against the same aggregate ceiling - provision outside this submission" }], sufficiency: "COMPLETE" },
        { localRef: "r8b", sourceSectionRef: "9.06(b)", covenantFamily: "INDEBTEDNESS", ruleType: "QUANTITATIVE_PERMISSION", posture: "PERMISSION", action: "GUARANTEE", capacityExpression: { kind: "LEDGER_USAGE_REFERENCE", sharedCapRef: "sc1", citation: "§9.06(b)" }, dependsOn: [{ relationshipType: "SHARES_CAPACITY_WITH", targetRef: "r8a", description: "shares the 12% aggregate ceiling with clause (a)" }], sufficiency: "COMPLETE" },
      ],

/**
 * Phase 3F.1.5.3 — Section 24 cross-dataset generality regression. Re-runs
 ...
 * writing the result under docs/evaluation-v2-final-resolution/ instead of
...
async function main() {
  ...
  const payload = {
    schemaVersion: "1.0",
    evaluationVersion: "phase-3f1-5-3-final-evaluation-v2-resolution.v1",
    artifactId: "PHASE_3F1_5_3_CROSS_DATASET_REGRESSION",
    generatedAt: new Date().toISOString(),
    purpose: "Section 27: after Workstream A's matcher remediation is frozen, rerun evaluator regression on FWRG/LSB/CONMED without tuning, to confirm the sibling/sub-provision claim-identity fix is genuinely general (no package-specific/term-specific branches were introduced) and causes no unexplained regression on these permanent regression datasets.",
    ...
    priorFrozenBaselineForComparison: "docs/evaluation-v2-iteration-2/13-cross-dataset-regression.json (Phase 3F.1.5.2, byte-identical, untouched by this phase).",
    engineChangesThisPhase: [
      "claim-identity.ts (NEW): compareClaimIdentity() - purely structural sibling/sub-provision comparison via splitSectionRef",
      "types.ts: new I_CLAIM_IDENTITY dimension, new SIBLING_CLAIM_MISMATCH conflict code",
      "conflicts.ts: new conflict-detection block recording SIBLING_CLAIM_MISMATCH (NON_MATERIAL_VARIANCE) when GT and candidate are structural siblings under the same parent section",
      "semantic-correspondence.ts: I_CLAIM_IDENTITY dimension added to assessDimensions(); evaluatePair() now routes a sibling-ambiguous pair to INDETERMINATE (semantic-judge review) BEFORE the core-dimension full/partial gate, rather than letting shared family vocabulary resolve it deterministically",
    ],
    datasets: perDataset,
  ...
  const outPath = join(repoRoot, "docs/evaluation-v2-final-resolution/21-cross-dataset-regression.json");
  const body = `${JSON.stringify(payload, null, 2)}\n`;
  writeFileSync(outPath, body);
  console.error(`Wrote docs/evaluation-v2-final-resolution/21-cross-dataset-regression.json sha256=${contentHash(payload)} bytes=${Buffer.byteLength(body)}`);
}

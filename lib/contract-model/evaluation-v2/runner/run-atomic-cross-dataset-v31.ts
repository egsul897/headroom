/**
 * Evaluation Contract V3.1 — cross-dataset regression after the AMB-1 atomic-surfacing resolution. Applies
...
/**
 * V3.1 re-run. Identical computation to run-atomic-cross-dataset.ts, writing
 * under docs/phase-3-final-closure-resolution/ instead of the frozen V3
 * artifact, which must never be overwritten. The only behavioural difference
 * comes from deriveAtomicContract() itself now applying the atomic surfacing
 * contract (see surfacing-scope.ts).
 */
...
async function main() {
  ...
  const payload = {
    schemaVersion: "1.0",
    evaluationVersion: "evaluation-contract-v3.1.v1",
    artifactId: "ATOMIC_CONTRACT_CROSS_DATASET_REGRESSION_V31",
  ...
  const outPath = join(repoRoot, "docs/phase-3-final-closure-resolution/16-cross-dataset-regression.json");
  const body = `${JSON.stringify(payload, null, 2)}\n`;
  writeFileSync(outPath, body);
  console.error(`Wrote docs/phase-3-final-closure-resolution/16-cross-dataset-regression.json sha256=${contentHash(payload)} bytes=${Buffer.byteLength(body)} falseCreditRegression=${falseCreditRegressionDetected}`);
}

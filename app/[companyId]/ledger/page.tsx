...
/**
 * The Ledger tab (task "MAKE THE UI MATCH THE PROTOTYPE EXACTLY" -
 * reference/headroom-coherent.jsx's Ledger tab, including the illustrative
 * officer's compliance certificate). Generalized off app/ledger/page.tsx
 * (Coherent-only) - same real RP-pool summary (lib/covenant-engine.ts's
...
export default async function LedgerPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const { data, position } = positionResult;
  ...
  for (const doc of rpDocs) {
    ...
    for (const step of doc.rpWaterfall!.steps) {
      ...
      rpStats.push({ label: `${provision?.basketName ?? step.code} left`, value: sim.stepCapacitiesRemaining[step.code] ?? 0 });
    }
  }
  ...
  return (
    <div className="stack">
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div className="card-subtitle">Illustrative only — computed live from the same position data as the Dashboard tab, not tied to any specific section numbers.</div>
          </div>
        </div>
        <Disclosure closedLabel="Generate draft" openLabel="Hide draft">
          <div style={{ border: "1px solid var(--line)", borderRadius: 4, background: "#fffefb", padding: "22px 20px" }}>
            ...
            <div className="serif" style={{ fontSize: 14, lineHeight: 1.65, marginTop: 14 }}>
              The undersigned officer certifies, as of the latest financial snapshot: (i) the Total Net Leverage Ratio was <span className="mono">{fmtX(position.metrics.totalNetLeverage)}</span>; (ii) the Fixed Charge
              Coverage Ratio was <span className="mono">{fmtX(position.metrics.fixedChargeCoverage)}</span>; (iii) the Senior Secured Net Leverage Ratio was <span className="mono">{fmtX(position.metrics.seniorSecuredNetLeverage)}</span>;
              (iv) no Default or Event of Default has occurred and is continuing under any Debt Document.
            </div>
            ...
            {debtSchedule.map((e) => (
              <div key={e.id} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px dotted var(--line)" }}>
                <span className="serif" style={{ fontSize: 13.5 }}>
                  {e.description}
                </span>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20, alignItems: "flex-end" }}>
              <div>
                ...
                <div className="serif muted" style={{ fontSize: 12, marginTop: 3 }}>
      ...
      <Card>
        ...
        <form action={addLedgerEntry.bind(null, companyId)} style={{ display: "grid", gap: 10 }}>

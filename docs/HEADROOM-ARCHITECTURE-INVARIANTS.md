...
37. **NO_SILENT_MATERIAL_FAILURE: when Headroom encounters a potentially material contractual claim it cannot safely convert into trusted semantic state, that fact must be preserved as an explicit, persisted, claim-specific production record — never reconstructed after the fact from nearby pipeline artifacts, and never inferred indirectly by an evaluator.** This generalizes invariant 9 (unsupported semantics must be surfaced) from "the AI's own turn-level output must say so" to "the fact must survive as durable, queryable, resolvable state downstream of that turn." Three genuinely distinct outcomes exist for a material claim: (a) semantic success — safely represented and (where verified) confirmed; (b) explicit safe failure — Headroom knows and records exactly which claim it could not safely handle; (c) silent failure — the claim disappears with no trace. Only (a) and (b) are acceptable; (c) is the one outcome this invariant exists to make structurally impossible. Explicit safe failure is a safety mechanism, not a substitute for semantic competence — the required hierarchy is UNDERSTAND CORRECTLY > FAIL SAFELY AND SPECIFICALLY > SILENT FAILURE, never "when uncertain, flag everything." An explicit safe-failure record for one claim never satisfies another claim, however similar — a sibling's flag, a generic section-level warning, or structural proximity never substitutes for a claim-specific record (this is invariant 20's identity discipline applied to safety state, not just semantic correspondence). (Concrete precedent and origin: Phase 3F.1.5.3's Evaluation Contract V2 taxonomy study and Phase 3F.1.5.R's Evaluation Contract V3 both found that asking an evaluator to reconstruct "was this specifically surfaced" from indirect pipeline evidence is not reliably reproducible even among independent human-equivalent reviewers — Evaluation Contract V3's own `surfacingStatus` dimension reproduced at only 74.1% among reviewers and 66.7% between the evaluator and reviewer consensus, both far below the 90% safety-critical threshold. The correct fix was not another evaluator methodology; it was moving the fact itself onto the production side, where Phase 3F.1.5.R's `ClaimReviewItem` architecture records it directly.)

...
> **SUPERSEDED (2026-08-25, later the same day — "Final legal review status instruction"):** this document's central conclusion — that a single-reviewer founder confirmation could not, under the then-controlling two-reviewer policy, promote `GoldenTest.status` to `FOUNDER_AND_PEER_REVIEWED` — no longer applies. The founder subsequently abolished the two-reviewer requirement outright: his own review is now Headroom's complete legal-verification standard, and the enum value was renamed to `VERIFIED`. All 48 rows this document describes as intentionally left `UNVERIFIED` (§6's "metadata/status-model mismatch") are now `VERIFIED` (`scripts/finalize-founder-sole-review-verified-2026-08-25.ts`), and the 2 rows reverted in §4 below (rows 16/17) are `VERIFIED` again too — this time on the basis that legal review and the engineering discrepancy §3 identifies are separate dimensions, not because the discrepancy was resolved (it wasn't). See `docs/legal-review-status-model.md` §0/§10 for the current, controlling policy and the worked example. **The rest of this document — the engineering-gap finding in §3, and the specific reconciliation actions in §4 — is left exactly as originally written; only the review-status conclusion above it is superseded.**

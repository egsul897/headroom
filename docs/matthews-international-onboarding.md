...
## §K — Re-check (2026-08-25): document-wide, substance-based search for a debt-incurrence covenant

**Status of this section: append-only.** Nothing above is deleted or edited. This section was added by a separate, later task whose sole purpose was to re-examine §B.1's "no negative covenant restricting Indebtedness incurrence" finding, because that finding's own search — read literally — was anchored to "Article VI" / "§6.0x," the section number the Indenture happens to use for its own equivalent restriction, rather than a search of the Credit Agreement's *entire* text for the *substance* of a debt-incurrence restriction regardless of numbering. Legal review of Matthews remains a separate, later phase, not begun here — this section, like §B.1 before it, is engineering-verified, primary-source-cited, and `UNVERIFIED`.

### K.1 — Original claim (unchanged, quoted for reference)

> "the Credit Agreement contains **no negative covenant restricting Indebtedness incurrence at all.** Article VI ('Negative Covenants') runs §6.01 (Liens) → ... → §6.08 (Dividends and Distributions). There is no '§6.0x Indebtedness' section, in the original 2020 text or in any of the six amendments read." (§B.1, above)

### K.2 — Methodology now used (document-wide, substance-based, not section-anchored)

All source documents were re-fetched directly from EDGAR this session (`https://www.sec.gov/Archives/edgar/...`, `User-Agent: Headroom Research research@example.com`), independent of the prior pass's cache:

| Document | Accession | Exhibit/file | Role in this re-check |
|---|---|---|---|
| Third A&R Loan Agreement (base) | 0000063296-20-000040 | `ex101loanagreement.htm` | 2020 original full text |
| First Amendment | 0000063296-21-000048 | `firstamendmenttothirdamend.htm` | Redline, re-fetched |
| **Second Amendment** | **0000063296-22-000060** | **`secondamendmenttothirdamen.htm`** | **Newly located this session** — filed as Ex. 10.1 to the 10-Q for the quarter ended 6/30/2022 (filed 2022-07-29). The prior pass's §A table flagged this exhibit as "not independently isolated (recital-confirmed only)"; it is now independently fetched and read. It is a full restated-agreement exhibit (has its own Table of Contents), not a narrow redline. |
| Third Amendment | 0000063296-22-000099 | `thirdamendmenttothirdamend.htm` | Re-fetched |
| Fourth Amendment | 0000063296-23-000044 | `fourthamendmenttothirdamen.htm` | Re-fetched — full restated-agreement exhibit |
| Fifth Amendment | 0000063296-24-000010 | `fifthamendmenttothirdamend.htm` | Re-fetched — full restated-agreement exhibit (used as the primary "current-through-Amendment-5" text below, since it is the most recent FULL restatement prior to the Sixth Amendment's narrower redline) |
| Sixth Amendment | 0001193125-24-224129 | `d872508dex102.htm` | Re-fetched — confirmed a narrow, targeted redline only (§1.01, §5.03, §5.12, §5.15, §6.01, §6.04, §6.07, §7.01(h)/(i)) |
| Indenture (2024 Notes) | 0001193125-24-228450 | `d873825dex41.htm` | Re-fetched in full, for the cross-reference check (§K.6) |

**Located via EDGAR full-text search** (`efts.sec.gov/LATEST/search-index?q="Third Amended and Restated Loan Agreement"&ciks=0000063296`), which also surfaced two amendments filed *after* the Sixth Amendment and after the prior pass's own research cutoff — see §K.7.

**Step 1 — full Table of Contents, every version.** The complete Article/Section listing (Articles I–IX) was read from: the 2020 base agreement, the Fifth Amendment's full restated text (current through Amendment 5, dated 1/31/2024), and the most recent full restatement found this session — the **Eighth Amendment** (filed 2026-02-17; see §K.7) — the version actually in force as of this report's own date (2026-08-25). In **every one** of these three full-text versions, Article VI ("Negative Covenants") runs identically: §6.01 Liens → §6.02 Restrictions on Non-Loan Party Subsidiaries → §6.03 Self-Dealing → §6.04 Disposition of Assets → §6.05 Margin Stock → §6.06 Partnerships; Mergers or Consolidation; Acquisitions → §6.07 Double Negative Pledge → §6.08 Dividends and Distributions. No amendment, at any point in the chain, inserted, renumbered, or retitled a section into Article VI. Article V ("Affirmative Covenants") likewise runs identically 5.01–5.17 in all three versions, with §5.14 "Financial Covenants" in the same position throughout.

**Step 2 — document-wide keyword search**, run against the Fifth Amendment's full restated text (the fullest available single copy of the operative agreement) with each keyword's occurrences mapped to the Article they fall in, not just counted:

| Keyword | Total hits | Article I (Defs) | Art. II | Art. III | Art. IV | **Art. V** | **Art. VI** | Art. VII | Art. VIII | Art. IX |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| "Indebtedness" | 77 | 33 | 6 | 4 | 2 | 0 | 10 | 11 | 7 | 3 |
| "incur" | 34 | 13 | 7 | 2 | 0 | 3 | 2 | 0 | 2 | 5 |
| "assume" | 16 | 5 | 1 | 0 | 0 | 0 | 2 | 0 | 8 | 0 |
| "guarantee" (verb/noun forms) | 13 | 12 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 1 |
| "Financial Covenant" | 6 | 3 | 0 | 0 | 0 | 3 | 0 | 0 | 0 | 0 |
| "Leverage Ratio" | 18 | 4 | 8 | 0 | 0 | 6 | 0 | 0 | 0 | 0 |
| "Interest Coverage" | 3 | 1 | 0 | 0 | 0 | 2 | 0 | 0 | 0 | 0 |
| "springing" | 0 | — | — | — | — | — | — | — | — | — |

Every "Indebtedness" occurrence outside Article I/V/VI was individually read, not just counted: Article II's six hits are payment-application/cross-Foreign-Borrower-liability boilerplate; Article III's four hits are the Use-of-Proceeds representation and AML/sanctions representations (representations *about* debt, not restrictions *on* it); Article IV's two hits are a closing condition ("(j) Repayment of Prohibited Indebtedness. All Indebtedness not permitted under Section 6.02 shall have been paid in full" — a one-time 2020 closing condition that itself cross-references §6.02, not an independent restriction); Article VII's eleven hits are the setoff/equalization/proceeds-waterfall mechanics; Article VIII's seven hits are Agent-indemnification boilerplate. None is a debt-incurrence restriction. "springing" returns zero hits anywhere — Matthews' §5.14 tests are quarterly maintenance covenants tested regardless of revolver utilization, not a utilization-triggered springing mechanic.

**Step 3 — the defined term "Permitted Additional Indebtedness."** This is the one genuinely new item this document-wide pass surfaces that the prior, Article-VI-anchored pass did not name. Full text (Article I): *"'Permitted Additional Indebtedness' shall mean unsecured notes and/or guarantees thereon contemplated to be and/or actually issued by the Borrower and its Affiliates (i) the covenants, events of default, subsidiary guarantees and other material terms of which, taken as a whole, are on customary market terms... (ii) the obligors with respect to which are... (iii) so long as both before and after giving effect to the incurrence of which... no Potential Default or Event of Default has occurred and is continuing."* This term is **operatively used**, not an orphaned defined-and-unreferenced term: it appears as a carve-out in §6.02 (Restrictions on Non-Loan Party Subsidiaries — "other than... (b) in connection with the 2017 Note Offering, the 2024 Note Offering, Permitted Additional Indebtedness or Receivables Financing") and in §6.07 (Double Negative Pledge — "a Loan Party... may enter into such agreement: (i) in connection with any Permitted Additional Indebtedness or the 2024 Note Offering..."). But its operative use in both places is as an **exception/carve-out to a *different* covenant** (the anti-restrictive-agreement covenant and the negative-pledge covenant, respectively) — **not** as a dollar- or ratio-capped gate on the *amount* of such Indebtedness a Loan Party may incur. The definition itself contains no dollar cap, no percentage-of-EBITDA cap, and no leverage-ratio test — only qualitative conditions (customary terms, correct obligor class, no Default at incurrence). This is the second of the task's two named cases — a defined term that *is* operatively referenced elsewhere, as distinct from an orphaned one — and it **confirms, rather than contradicts**, the original finding: the drafters plainly contemplated Matthews issuing more debt (they built a whole defined term and two operative carve-outs around exactly that) and still chose not to cap it by dollar amount or ratio anywhere in the Credit Agreement.

### K.3 — §5.14 Financial Covenants, full primary text (task item 3)

Read in full from the 2020 base text, the Fifth Amendment's restated text, and the Eighth Amendment's restated text (§K.7) — identical in structure throughout, confirming the prior pass's own note that the Sixth Amendment does not touch §5.14 (`grep`-confirmed: zero occurrences of "5.14" anywhere in the Sixth Amendment's text).

§5.14 contains **three** tests, not the two the prior pass reported — the original document's §B.1.2 named only the Leverage Ratio and Senior Leverage Ratio; it omitted **§5.14(b), Interest Coverage Ratio**, which is corrected here:

- **§5.14(a)(i) Leverage Ratio** — step-down schedule from 5.00x (through 6/30/2020) to 4.75x (through 12/31/2020) to a permanent **4.50x** floor thereafter (originally effective from the Fiscal Quarter ending 3/31/2021; the Fifth Amendment's own text shows this date extended to 12/31/2023, a redline visible in the fetched exhibit — the *level*, 4.50x, is unchanged, only the date it becomes permanent moved out).
- **§5.14(a)(ii) Senior Leverage Ratio** — same step-down shape, permanent floor **3.50x**.
- **§5.14(b) Interest Coverage Ratio** — *"The US Borrower and its Subsidiaries shall maintain an Interest Coverage Ratio... greater than or equal to 3.00 to 1.0"* — present, unchanged, in the 2020 base text and every subsequent full restatement read.
- Both (a) tests carry a one-time "Temporary Leverage/Senior Leverage Increase Period" relief (to 5.00x/4.00x for four quarters) available once, on written notice, following a Permitted Acquisition.

All three are periodic **maintenance** tests — measured against the Company's actual then-current ratios each Fiscal Quarter-end, with breach an Event of Default independent of any specific transaction — not an incurrence-conditioned precondition tested only when a specific new-debt transaction is proposed (contrast the Indenture's §4.09, which *is* incurrence-conditioned: "may Incur Debt if... on a Pro Forma Basis"). §5.14 contains no "may Incur additional Indebtedness if" clause anywhere in its text; it only ever says what ratio the Company "shall maintain." Nothing in §5.14, read in full, functions as a precondition gating a *specific* new-debt transaction the way the Indenture's ratio debt basket does — it can only ever produce a covenant *default* (an Event of Default under §7.01, triggering acceleration/remedies), never a same-transaction "no" to a specific borrowing the way an incurrence covenant would. This confirms, rather than narrows, §B.1.2's original category distinction — with the correction that the maintenance package Matthews actually agreed to is three ratios, not two.

### K.4 — Article I definitions vs. operative gates (task item 2, third example)

Two contrasting, fully-verified cases from the same document:

1. **"Indebtedness" itself** — defined at length in Article I (a standard, comprehensive definition: borrowed money, guarantees, letter-of-credit reimbursement obligations, Capital Lease obligations, Hedging Obligations, etc.) and used constantly throughout the document (77 hits, §K.2) — but **never gated by an operative restrictive covenant that limits how much of it a Loan Party may incur**, anywhere outside §6.01 (Liens, which restricts *securing* Indebtedness, not incurring it) and §6.02/§6.07 (restrictions on *contractual restrictions*, not on the Indebtedness itself). This is the "defined term never actually gated" case the task anticipated, and it is what is actually true here.
2. **"Permitted Additional Indebtedness"** — the operatively-referenced-but-uncapped case, per §K.2 Step 3 above.

Both point the same direction: the term "Indebtedness" is not merely absent from the document (it appears constantly) — it is comprehensively *defined* and *used*, but never made the subject of an amount-restricting operative covenant anywhere in the Credit Agreement's nine Articles.

### K.5 — §5.14 as a practical, indirect cap (task item 3, restated)

Could §5.14's Leverage Ratio maintenance test *practically* limit new debt issuance, even without being styled as an incurrence covenant? As a matter of economic effect, yes in the trivial sense any maintenance covenant does — a Company that borrows enough to push its Leverage Ratio above 4.50x (or Senior Leverage above 3.50x, or Interest Coverage below 3.00x) breaches §5.14 and is in default, which as a practical matter deters borrowing past that point. But this is qualitatively different from an incurrence covenant (of the kind the Indenture's §4.09 is) in a way that matters to the golden-question set: an incurrence covenant is tested *at the moment of the specific transaction* and, if it fails, blocks *that transaction* from closing under the document's own terms (a `TransactionStatus` of `"blocked"`, in this codebase's own vocabulary). A maintenance covenant does not block the borrowing itself — the Company can (as a factual, mechanical matter) still draw the funds; breach is a separate, later-triggered default under §7.01, remediable by waiver, cure, or acceleration, and does not by its own terms prevent the transaction from happening. This is exactly why the codebase's `Permission` model — "what may be done, subject to clearing a precondition" — does not fit §5.14, and why no `Permission` row exists for it (§B.1.2, unchanged by this re-check).

### K.6 — Cross-document reference check (task item 4)

**The Indenture incorporates the Credit Agreement by reference into the Indenture's *own* debt-incurrence covenant — not the reverse.** Confirmed by direct text:

- Indenture Article I, **"Senior Credit Facility"**: *"means the Third Amended and Restated Loan Agreement, dated March 27, 2020, by and among the Company, Citizens Bank, N.A., as administrative agent, and the other banks party thereto... including by or pursuant to any agreement or agreements or instrument or instruments that extend the maturity of any Debt thereunder, or **increase the amount of available borrowings thereunder (provided that such increase in borrowings is permitted under Section 4.09)**..."* — the Indenture's own §4.09 (Limitation on Incurrence of Debt) is what gates any increase in the Credit Agreement's own borrowing availability, from the Notes' perspective.
- Indenture Article I, **"Debt Facilities"**: *"means one or more debt facilities (including, without limitation, the Senior Credit Facility)..."*, and Permitted Debt clause (1)(a): *"Debt of the Company or any Restricted Subsidiary Incurred pursuant to any Debt Facilities... in an aggregate principal amount at any one time outstanding not to exceed the sum of (a) $1,300.0 million and (b)..."* — this is the exact provision the original document already modeled as `ind_permitted_debt_1a_flat` (§B.2.1); this re-check confirms textually *why* that clause is the correct place to model Credit-Agreement-sourced borrowing capacity: the Indenture is the document imposing the quantitative cap on it, not the Credit Agreement imposing a cap on itself.
- **No reciprocal cross-reference exists.** The Credit Agreement's own text (read in full, §K.2) never mentions the Indenture, the 2024 Notes, "Debt Facilities" as a concept, or any incorporation of the Notes' own covenant package — its only awareness of the 2024 Note Offering is the narrow, lien-side cross-reference the Sixth Amendment added to §6.01(j)/§6.02/§6.07/§7.01(h) (already modeled, §B.1.1/§B.2.1). The gating relationship between the two documents is one-directional (Indenture → Credit Agreement's borrowing headroom), never the reverse.

### K.7 — Beyond the original claim's stated scope: two further amendments now exist

The original claim was scoped explicitly to "the Third Amended and Restated Loan Agreement, as amended through its Sixth Amendment." EDGAR full-text search this session (`efts.sec.gov/LATEST/search-index?q="Seventh Amendment"&ciks=0000063296`) surfaced that, as of this report's own date (2026-08-25), **two further amendments have since been executed** — a fact outside §B.1's original scope, not a correction to it, but material to flag rather than silently omit:

- **Seventh Amendment** — Closing Date **May 1, 2025** (per its own defined term, quoted in the Eighth Amendment's restated text: *"'Seventh Amendment Closing Date' shall mean May 1, 2025"*), concurrent with the Propelis/SGS joint-venture formation (Item 2.01, 8-K filed 2025-05-01, accession 0001193125-25-110083). That 8-K itself does not attach the Seventh Amendment as an exhibit (only the Contribution Agreement, an RSU amendment, and a press release). Its own accession/exhibit number was **not independently isolated** in this pass — recital-confirmed only, exactly the same disposition the original document already gave the Second Amendment before this re-check independently fetched it (§K.2). Immaterial to this section's conclusion (see below).
- **Eighth Amendment** — dated 2/11/2026, filed 2026-02-17 (accession 0001193125-26-052745, Ex. 10.1 `d100367dex101.htm`, a full restated-agreement exhibit). Per its own 8-K Item 1.01 text: reduces the revolver to $700.0M (from $750.0M, with further step-downs tied to specified asset dispositions/JV distributions), increases the term loan facility to $150.0M, **revises the §5.14(b) Interest Coverage Ratio schedule** (3.00x through 12/31/2025 → 2.50x for Q1 FY2026 → 2.75x for Q2 FY2026 → back to 3.00x from Q3 FY2026 on — confirmed directly from its own restated §5.14 text, §K.3 above), and modifies the "EBIT" definition (adds back 50% of cash dividends/distributions received from the Propelis Joint Venture).

**These are two further maintenance-covenant-style amendments, not a new negative covenant.** Critically, the Eighth Amendment's own full restated Table of Contents — independently read this session (§K.2, Step 1) — confirms Article VI still runs §6.01–§6.08 with the identical section titles, and Article V still runs 5.01–5.17 with §5.14 in the same position. **The "no debt-incurrence covenant" finding is confirmed to hold under the actual, current, most-recently-amended version of the Credit Agreement in force as of this report's own date — not merely the Sixth-Amendment-vintage version the original claim was scoped to.** This closes a staleness gap the original document's own "amendment chain reconstruction" (§A) did not anticipate (it was written before these two amendments existed) — noted here as a correction to that section's completeness, not to its substance.

### K.8 — Conclusion

**(a) Methodology**: full Table of Contents read across three independent full-text versions (2020 base, Fifth-Amendment-restated, Eighth-Amendment-restated/current); document-wide keyword search for "Indebtedness"/"incur"/"assume"/"guarantee"/"Financial Covenant"/"Leverage Ratio"/"Interest Coverage"/"springing," every hit mapped to its Article and, for hits outside Articles I/V/VI, individually read; the "Permitted Additional Indebtedness" defined term traced to both its own definition and its two operative uses; §5.14's complete primary text (all three tests) read across every full-restatement version; the Indenture's own cross-reference to the Credit Agreement traced and quoted.

**(b) Holds.** The original §B.1 finding — no negative covenant restricting Indebtedness incurrence anywhere in the Credit Agreement — is **confirmed, document-wide**, not merely within Article VI.

**(c) Sections confirming the absence, beyond Article VI**: Article I (definitions — "Indebtedness" fully defined, comprehensively used, never gated); Article II (borrowing-mechanics/payment-waterfall boilerplate only); Article III (representations only — Use of Proceeds, AML/sanctions); Article IV (a single historical 2020 closing condition, itself cross-referencing §6.02, not an independent restriction); §5.14 in Article V (three maintenance tests — Leverage Ratio 4.50x, Senior Leverage Ratio 3.50x, **Interest Coverage Ratio 3.00x** — none incurrence-conditioned, corrected here to include the third test the original pass omitted); Article VII (defaults/remedies mechanics only); Article VIII (Agent-indemnification boilerplate only); Article IX (miscellaneous, no hits of substance). The "Permitted Additional Indebtedness" defined term additionally confirms the finding by demonstrating the drafters contemplated future debt issuance and still chose not to cap it. The Indenture's own cross-reference into the Credit Agreement (§K.6) confirms the gating relationship runs one direction only, and it is not the Credit Agreement gating itself.

**(d)** Not applicable — the finding holds; no correction to the permission inventory is required.

**Additional, out-of-original-scope finding**: two further amendments (Seventh, Eighth) postdate the original claim's stated Sixth-Amendment scope; independently re-confirmed the finding still holds under the current, Eighth-Amendment-restated text as of 2026-08-25 (§K.7). No `Permission`/`PermissionRelationship`/`SharedCapacityConstraint`/`FinancialState`/`Facility`/`DebtEvent` row was created, modified, or deleted by this section — this is a text re-check only, consistent with this task's own constraints.

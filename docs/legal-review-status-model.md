...
| Financial-data provenance | `ExternalInputRecord` (`ExternalInputKind.CERTIFIED_EXTERNAL_INPUT`) | Covenant EBITDA — still **not** a certified row for Coherent |
...
A completed `FOUNDER_AND_PEER_REVIEWED` legal review **never** marks a financial input `CERTIFIED_EXTERNAL_INPUT`, never changes an `EvaluationStatus`/`PathStatus`, and never substitutes for either. A valid combination looks like: `LEGAL INTERPRETATION: FOUNDER_AND_PEER_REVIEWED / FINANCIAL INPUT: COVENANT EBITDA — CERTIFICATION PENDING / ENGINE EXECUTION: PASS`. Concretely for Coherent: the four load-bearing legal conclusions (§8) are `FOUNDER_AND_PEER_REVIEWED`, but Covenant EBITDA remains a plain, non-certified `FinancialSnapshot.ebitda` value with no `ExternalInputRecord` row — see `docs/coherent-legal-model-baseline-v1.md` §6.

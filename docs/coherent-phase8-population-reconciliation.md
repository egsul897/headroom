...
**What changed**: Coherent's $1,700M covenant EBITDA (`prisma/seed-data.ts` `COHERENT_DATA.financials.ebitda`) now carries a real, persisted `ExternalInputRecord` (id `coh-eir-covenant-ebitda-public-filing-reconstruction`, populated by `scripts/populate-coherent-ebitda-provenance.ts`) of kind **`PUBLIC_FILING_RECONSTRUCTION`** — a new `ExternalInputKind` value distinct from `CERTIFIED_EXTERNAL_INPUT` (see `prisma/schema.prisma`'s enum comment for the full definition). Its `sourceRef` cites the specific FY2026 Form 10-K lines the figure reconciles to: Net Earnings Attributable to Coherent Corp. ($804,998K) + Income Tax Expense ($60,849K) + Interest expense ($190,267K) + Depreciation ($241,561K) + Amortization ($280,334K) + Restructuring charges ($63,390K) + Impairment of assets held-for-sale ($64,404K) + Share-based compensation ($186,468K) − Gain on sale of business ($124,133K) − Gain on sale of equity investment ($73,998K) ≈ $1,694M, within ~0.3% of the seeded $1,700M. `reviewStatus: VERIFIED` on this row means "this reconstruction is genuinely traceable to real, cited public-filing line items" — not "certified to the dollar" (a ~$6M/0.3% residual gap is not independently closed; see the script's own header comment).
...
|---|---|---|---|
| Indenture secured max, standalone ($11,932.8M) | Yes | No — no First Lien SNLR, Reallocated Amount, or Contribution Indebtedness dependency (Contribution Indebtedness is unpopulated, so it is absent from this figure, not incorrectly included in it) | **LEGAL MODEL: `FOUNDER_AND_PEER_REVIEWED` / ENGINEERING: VERIFIED / FINANCIAL INPUT: `PUBLIC_FILING_RECONSTRUCTION`** — no remaining open gate of any kind |
| Indenture unsecured max, standalone ($11,932.8M) | Yes | No, same reasoning | Same as above |
| $100M/$500M/$1,000M secured or unsecured clear/blocked verdicts (§G row 4) | Yes | No — all tested amounts sit well inside base-formula capacity on both documents; none exercises the Reallocated Amount or First Lien SNLR paths | Same as above |
| Cross-document max secured ($5,130M) | Yes | **Yes** — CA-bound; the Credit Agreement's Cash-Capped Incremental is populated at base formula only (Reallocated Amount not enforced, §L) and the Ratio-Based Incremental's pari passu-secured prong depends on the unconfigured First Lien SNLR (§M/§S) | Unchanged: PROVISIONAL, and conservative (may understate) independent of EBITDA's status — the open gate here is engineering capability, not legal review or EBITDA provenance |
| Cross-document max unsecured ($5,130M) | Yes | Same CA-side caveat as above (CA is binding here too) | Unchanged, same reasoning |

...
**Every dollar figure in this document is PROVISIONAL — ENGINEERING-VERIFIED ONLY.** Outside counsel has NOT confirmed the load-bearing legal conclusions this population is built on (clause 6/24/25 non-netting, EBITDA addback-cap absence, Contribution Indebtedness availability, Collateral Suspension Period current-state determination). All source verification behind this document is internal, AI-driven review of primary EDGAR text — not independent legal confirmation. See §U for the full ledger. This constraint was given to this task as a precondition and is not re-derived here.

**Recommendation: `READY_WITH_FINANCIAL_INPUT_REVIEW_REQUIRED`.** `READY_TO_UPDATE_GOLDEN_TESTS` was explicitly unavailable for selection per the task's precondition and was **not** selected, regardless of how clean the reconciliation looks. See §T/§U.
...
| Load-bearing legal conclusion | Confirmation status | Figures depending on it |
|---|---|---|
...
**Every solver-native dollar figure in §E through §I of this document rests on at least one of the above unconfirmed conclusions and is therefore labeled PROVISIONAL — ENGINEERING-VERIFIED ONLY, not presented as a final answer.** This population is a shadow-run migration artifact for engineering review, not a covenant-compliance opinion.

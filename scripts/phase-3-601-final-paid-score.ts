 ...
 * phase-3-601-hd4-run.ts plus the frozen human reference set. Makes no model call and never re-runs the compiler.
 * The reference set is used here for the FIRST time in this mission - it never reached any prompt.
 * Run: npx tsx scripts/phase-3-601-final-paid-score.ts
 */
...
writeJson(`${OUT}/83-final-paid-reference-comparison.json`, {
  artifact: "PHASE 3 FINAL-BRIDGE / 6.01 FINAL RESTART-SAFE PAID RUN §20/§21/§22 - comparison against the 8 frozen Section 6.01 human reference items",
...
writeJson(`${OUT}/84-final-paid-quantitative-extra-audit.json`, {
  artifact: "PHASE 3 FINAL-BRIDGE / 6.01 FINAL RESTART-SAFE PAID RUN §23/§24 - quantitative audit and extra-output audit",
  at: new Date().toISOString(),
  quantitative: {
...
console.log(JSON.stringify({ counts: JSON.parse(readFileSync(`${OUT}/83-final-paid-reference-comparison.json`, "utf8")).counts, critical: tally(crit), material: tally(mats), extras: extraMaterialItems.length }, null, 1));

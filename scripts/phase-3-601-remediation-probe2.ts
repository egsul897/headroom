...
out.fromNodeRoute = { node: { nodeId: b1.nodeId, charStart: b1.charStart, chars: idx.getNodeText(b1.nodeId, "OWN").length }, refs: idx.findReferencesFrom(b1.nodeId, true).filter((r) => /2\.1[89]|2\.22|6\.01/.test(r.referenceText)).map((r) => ({ text: r.referenceText, normalized: r.normalizedTarget, resolved: r.resolved, targetNodeId: r.targetNodeId, ambiguous: r.targetAmbiguous, candidates: (r as unknown as { candidateNodeIds?: string[] }).candidateNodeIds?.length ?? null })) };

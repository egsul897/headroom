...
import { gradeRule, summarizeGrading, type ExpectedRuleShape, type SemanticErrorFinding } from "../lib/contract-model/compiler/semantic/grading";
...
async function main() {
  ...
  for (const c of CASES) {
    ...
    if (dryRun) {
      console.log(`  [dry run] operativeSourceText (${operativeSourceText.length} chars): ${operativeSourceText.slice(0, 200)}...`);
      console.log(`  [dry run] contextBundle items: ${contextBundle.items.length}, unresolvedDependencies: ${contextBundle.unresolvedDependencies.length}`);
      continue;
    }
    let result;
    try {
    ...
    // Grade: match each expectation to the best-fitting compiled rule/definition (by index order for multi-rule cases, or the sole definition).
    if (c.kind === "definition") {
      ...
      c.expected.forEach((exp, i) => {
        expectationsMap.set(exp.ref, exp);
        const findings = gradeRule(result.rules[i], exp);
        allFindings.push(...findings);
      });
    }
  }

...
const termNode = (termName: string): IRExpression => ({ kind: "DEFINED_TERM_REFERENCE", type: "MONEY", termName, companyId: "co", instrumentKey: "inst", resolvedDefinitionId: null, exprId: id() });
...
const ops = {
  ...
  AMBIGUOUS_BLOCKED_DEFINITION: summary(ev(termNode("blocked-term"), fixtureInputResolver({ definitions: [{ definitionId: "ir-definition:blocked", irSchemaVersion: "t", companyId: "co", instrumentKey: "inst", sourceDocumentId: "doc", termName: "blocked-term", covenantFamily: "DEFINITIONS_CALCULATION_RULES", calculationExpression: money(5), dependsOnTerms: [], sufficiency: "AMBIGUOUS", sufficiencyReasons: ["the Phase-3 layer could not resolve this term"], provenance: null, compilerVersion: null, sourceContentVersion: null }] }))),
...
// The two wall-clock scaling files live under tests/contract-model/, so they appear in the targeted run as well.
...
// Wall-clock scaling assertions over `segmentCoordinateClauses`, which lives in the FROZEN Phase-3 compiler tree.
// A new failing identity is attributed to load, not to Phase 4A, only when all four hold: the file is one of these
// timing files; the assertion is a wall-clock/scaling one; the file passes in isolation on recorded runs; and a
// direct measurement of the same function shows sub-quadratic growth.
...
const isolationAllPass = isolationRuns.length > 0 && isolationRuns.every((r) => r.failed === 0);
const scaling = process.env.SCALING_JSON && existsSync(process.env.SCALING_JSON) ? readJson<Any[]>(process.env.SCALING_JSON) : null;
const scalingSubQuadratic = scaling !== null && scaling.length > 0 && scaling.every((r) => r.subQuadratic === true);
...
const newFailingAllTiming = [...(newFailing ?? []), ...(targetedNew ?? [])].every(isTimingIdentity);
const onlyKnownFlake = newFailingAllTiming && isolationAllPass && scalingSubQuadratic && timedFunctionInFrozenTree;
const flakeCharacterisation = { newFailingIdentities: newFailing ?? [], newFailingIdentitiesInTargetedRun: targetedNew ?? [], allAreWallClockScalingAssertions: newFailingAllTiming, measuredFunction: "segmentCoordinateClauses (lib/contract-model/compiler/semantic-coverage/unit-hypothesis.ts)", measuredFunctionLastChangedAt: sh("git log --oneline -1 -- lib/contract-model/compiler/semantic-coverage/unit-hypothesis.ts"), measuredFunctionInsideFrozenPhase3Tree: timedFunctionInFrozenTree, isolationRuns, isolationAllPass, directMeasurement: scaling, directMeasurementSubQuadratic: scalingSubQuadratic, fullSuiteGrewBy: full && base ? full.tests - base.tests : null, attribution: onlyKnownFlake ? "NOT_ATTRIBUTABLE_TO_PHASE_4A: the measured production code is bit-identical to the Phase-3 baseline and passes in isolation; Phase 4A added tests to the shared parallel runner, which is what moved these wall-clock ratios" : "UNEXPLAINED" };
...
const gate: [number, string, boolean, string][] = [
  ...
  [24, "no Phase-3 semantic regression", Object.values(suites).every((s: Any) => s.newFailingVsBase === 0) && targetedNew !== null, `11 phase3Suites: ${Object.entries(suites).map(([k, v]: [string, Any]) => `${k} ${v.tests} tests, ${v.newFailingVsBase} new`).join("; ")}`],
  [25, "no new failing identities attributable to Phase 4A", targetedNew !== null && newFailing !== null && onlyKnownFlake, `targeted new ${targetedNew?.length ?? "n/a"}; full new ${newFailing?.length ?? "n/a"}, all wall-clock scaling assertions over frozen-tree code that pass in isolation and measure sub-quadratic: ${onlyKnownFlake}`],

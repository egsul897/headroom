 ...
 * Run: npx tsx scripts/phase-3-601-clean-score.ts
 */
...
writeJson(`${OUT}/30-clean-rerun-reference-comparison.json`, {
  artifact: "PHASE 3 FINAL-BRIDGE / 6.01 CLEAN RERUN §18/§19/§20 - comparison against the 8 frozen Section 6.01 human reference items",
...
writeJson(`${OUT}/31-clean-rerun-quantitative-extra-audit.json`, {
  artifact: "PHASE 3 FINAL-BRIDGE / 6.01 CLEAN RERUN §21/§22 - quantitative audit and extra-output audit",
  at: new Date().toISOString(),
  quantitative: {
...
console.log(JSON.stringify({ counts: JSON.parse(readFileSync(`${OUT}/30-clean-rerun-reference-comparison.json`, "utf8")).counts, critical: tally(crit), material: tally(mats), extras: extraMaterialItems.length }, null, 1));

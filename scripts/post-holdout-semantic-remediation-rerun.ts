/**
 * POST-HOLDOUT-SEMANTIC-REMEDIATION re-run - the EXACT SAME frozen A1-A6/
 * B1-B4 semantic holdout as scripts/final-phase3-closure-holdout-run.ts
 * (byte-identical REGIONS/DOCS/SRC_DIR/GT/thresholds/provider-model
 * resolution), re-executed against the post-remediation frozen production
 * code (REMEDIATED_PRODUCTION_SHA db7f32a) to measure whether Unit A/Unit B
 * actually lift the substantive-capture rate. Output goes to a SEPARATE
 * directory (post-holdout-semantic-remediation-rerun) so the original,
 * pre-fix holdout evidence (final-phase3-closure-holdout-run) is preserved
 * unmodified for before/after comparison. No GT modification, no new
 * examples added, no region/claim re-selection.
 */
...
const OUT_DIR = "tests/fixtures/unseen-packages/post-holdout-semantic-remediation-rerun";
const BUDGET_CEILING_USD = 10.0;

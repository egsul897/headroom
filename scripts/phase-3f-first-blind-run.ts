/**
 * Phase 3F - the frozen first-blind run against the genuinely unseen DSGR
 * package (task §7/§8). Runs Phase 2A (structural) -> 2B (discovery,
 * real model calls) -> 2C (package graph, zero LLM calls) -> 2D (context
 * retrieval, zero LLM calls) -> 2G (amendment pipeline + operative state,
 * bounded real model calls) -> 3B (compilation, capped per the pre-
 * committed deterministic bounding rule in
 * tests/fixtures/unseen-packages/phase-3f-freeze/phase-3f-cost-estimate-and-authorization.json)
 * -> 3C (verification of every compiled candidate) -> 3E (whole-package
 * semantic coverage audit, Layers A/B only - no Layer C AI inventory, to
 * keep this run inside the authorized $30 ceiling).
 *
 * NO MANUAL COVENANT TARGETS, NO MANUAL DEFINITION SELECTION, NO MANUAL
 * AMENDMENT LINKING, NO MANUAL SECTION SELECTION FOR COMPILATION beyond
 * the single pre-committed, content-blind ordering rule already recorded
 * BEFORE this script was written. Every stage's real output is persisted
 * incrementally as it completes, so a mid-run failure never loses
 * already-purchased real work.
 *
 * Run via: npx tsx --env-file=.env.local scripts/phase-3f-first-blind-run.ts
 */
...
const COMPILE_CAP = 30;
const BUDGET_CEILING_USD = 30;

const SRC_DIR = "tests/fixtures/unseen-packages/dsgr-2022-2025-credit-facility/extracted-text";
const OUT_DIR = "tests/fixtures/unseen-packages/phase-3f-first-blind-run";
...
async function main() {
  ...
  console.log("================ PHASE_3F_FIRST_BLIND_RUN ================");
  console.log(`Started: ${new Date().toISOString()}`);
  console.log(`Budget ceiling: $${BUDGET_CEILING_USD}`);

  // ============================================================
  // STAGE 1: Phase 2A structural indexing (deterministic, $0)
  // ============================================================
  ...
  if (discoveryCaller.isSynthetic) throw new Error("FATAL: no real credential detected - refusing to proceed with a synthetic first-blind run.");
  ...
  // ============================================================
  // STAGE 4: Phase 2D context retrieval (deterministic, $0)
  // ============================================================
  ...
  // ============================================================
  console.log("\n=== STAGE 5: Phase 2G amendment pipeline + operative state (bounded real model calls) ===");
  ...
  // ============================================================
  // STAGE 6: Phase 3B compilation (capped per the pre-committed rule)
  // ============================================================
  ...
  if (compileCaller.isSynthetic) throw new Error("FATAL: no real credential detected for the semantic compiler - refusing to proceed with a synthetic first-blind run.");
  ...
  // ============================================================
  // STAGE 8: Phase 3E whole-package semantic coverage audit
  // ============================================================
  ...
  const finalSummary = {
    runId: "PHASE_3F_FIRST_BLIND_RUN",

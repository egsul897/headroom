...
import { PER_CANDIDATE_TIMEOUT_MS, buildInput, callerFor, maxTokensFor, prepare, record, withTimeout } from "./compile-run";
...
async function main() {
  ...
  const raw = catalogue.find((m) => m.id === VALIDATION_MANIFEST.model)!;
  ...
  for (const spec of VALIDATION_MANIFEST.candidates) {
    ...
    if (ledger.mustStop(reservation)) { console.log("budget guard: stopping before the ceiling"); break; }
    ...
    const verification = await verifyCompiledCandidate({ compilerInput: input, compilationResult: result });
  ...
  fs.writeFileSync(path.join(OUT, "summary.json"), JSON.stringify({ manifest: VALIDATION_MANIFEST, results, budget: ledger.snapshot() }, null, 2));
}

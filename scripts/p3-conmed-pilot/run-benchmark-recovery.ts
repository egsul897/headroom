...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  const planRecord = {
    ...
    targetCount: loopCandidates.length, excluded: { ref: EXCLUDED_REF, reason: "already has complete current-pipeline evidence" },
  ...
  console.log(`PASS 1: ${loopCandidates.map((c) => c.ref).join(", ")}`);
  const pass1 = await runPass(1, loopCandidates);
  ...
  save("03-run-manifest", {
    ...
    spend: { exactUsd: ledger.exactSpendUsd, timeoutReservationsRetainedUsd: ledger.retainedUnknownUsd, outstandingReservedUsd: ledger.outstandingReservedUsd, committedUsd: ledger.committedUsd, remainingUsd: ledger.remainingUsd, ceilingUsd: RECOVERY_CEILING_USD, stopAtUsd: RECOVERY_STOP_AT_USD, preflightUsd: preflight.spendUsd, amendmentPipelineUsd: amendmentUsd },
  });

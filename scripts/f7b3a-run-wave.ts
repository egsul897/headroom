/**
 * F-7B.3A - PAID, hard-capped wave runner. Executes exactly one committed wave (A or B) of the frozen F-7B.3
...
import { F7B3_DIR, STAGE2_CAP_USD, costModel, estimateShardUsd, loadFrozenStage1, loadStage2, stage2EvidencePath, stitchAll } from "./f7b3-lib";

const STARTING_SHA = "b1980baf529809a7c02eadb1294a72ba91257aed";
const MANIFEST = "docs/phase-3-remediation-f7b3/00-baseline-and-remaining-manifest.json";
const wave = (process.argv[process.argv.indexOf("--wave") + 1] ?? "").toUpperCase();
if (wave !== "A" && wave !== "B") { console.error("usage: --wave A|B  (Wave C must not execute in this mission)"); process.exit(2); }
...
(async () => {
  ...
  if (!Object.values(baseline).every((b) => b.pass)) {
    writeJson(`${F7B3_DIR}/10-baseline-invalid.json`, { artifact: "F-7B.3A §1 baseline check failed - zero paid calls", at: new Date().toISOString(), baseline, verdict: "F7B_3A_BASELINE_INVALID" });
    console.error("F7B_3A_BASELINE_INVALID"); process.exit(4);
  }
  ...
  ledger.missionCapUsd = STAGE2_CAP_USD; ledger.stage1CapUsd = STAGE2_CAP_USD;
  ...
  const remainingCap = STAGE2_CAP_USD - ledger.spentUsd;
  ...
  if (!precheck.fitsCap || precheck.fitsBalance === false) {
    writeJson(`${F7B3_DIR}/1${wave === "A" ? 1 : 5}-wave-${wave.toLowerCase()}-cost-bound.json`, { artifact: `F-7B.3A §4/§10 Wave ${wave} could not start within the cap or balance`, at: new Date().toISOString(), precheck, verdict: wave === "A" ? "F7B_3A_COST_BOUND_BEFORE_START" : "F7B_3A_COST_BOUND_AFTER_WAVE_A" });
    console.error(wave === "A" ? "F7B_3A_COST_BOUND_BEFORE_START" : "F7B_3A_COST_BOUND_AFTER_WAVE_A"); process.exit(0);
  }
  ...
  for (const shard of targets) {
    ...
    const remaining = STAGE2_CAP_USD - ledger.spentUsd;
    if (remainingWave > remaining) {
      ledger.refusals.push({ stage: 2, shardId: shard.shardId, kind: "PRE_SHARD", estimatedUsd: remainingWave, spentUsd: ledger.spentUsd, capUsd: STAGE2_CAP_USD, at: new Date().toISOString() });
      saveLedger(ledger);
  ...
  const gate = {
    ...
    costTrajectory: { spentUsd: +ledger.spentUsd.toFixed(4), capUsd: STAGE2_CAP_USD, remainingUsd: +(STAGE2_CAP_USD - ledger.spentUsd).toFixed(4), costBounded, pass: costBounded === null },
  };
  ...
  const nextRows = manifest.manifest.filter((m) => m.wave === (wave === "A" ? "B" : "C"));
  const nextEstimate = { wave: wave === "A" ? "B" : "C", shards: nextRows.length, renderedTokens: nextRows.reduce((a, m) => a + m.estimatedFirstTurnInputTokens, 0), atWaveMeanRateUsd: +nextRows.reduce((a, m) => a + m.estimatedFirstTurnInputTokens * actualRate, 0).toFixed(4), atWaveWorstRateUsd: +nextRows.reduce((a, m) => a + m.estimatedFirstTurnInputTokens * worstActualRate, 0).toFixed(4), conservativeUsd: +nextRows.reduce((a, m) => a + m.estimatedFirstTurnInputTokens * worstActualRate * 1.25, 0).toFixed(4) };

  const creditsAfter = await gatewayCredits();
  writeJson(`${F7B3_DIR}/1${wave === "A" ? 2 : 6}-wave-${wave.toLowerCase()}-ledger-and-gate.json`, {

...
// HD-6 FIX (PHASE 3 / 6.01 remediation §20): the canonical production shape is read through ONE reader
// (scripts/phase-3-601-trust-read.ts). execution.sharded.contextualEmissions is a COUNT of contextual emissions
// DETECTED (all demoted); the violation counter is execution.sharded.contextualEmissionsCredited (§18). A counter whose
// field is absent is NOT_MEASURABLE - never a silent zero, never substituted from another field.
const trustRead = readShardTrust(compile);
const contextualEmissionCount: number = trustRead.sharded?.contextualEmissionsDetected ?? 0;
const contextualCredited: number | null = trustRead.counters.contextualOwnershipCreditViolations.value;
...
const trust = [
  ...
  { counter: "contextual ownership-credit violations", ...m(compiled ? contextualCredited : null, compiled && contextualCredited !== null, contextualEmissionCount > 0 || (sh?.collisions ?? 0) > 0), rule: `contextual emissions CREDITED into the stitched IR (execution.sharded.contextualEmissionsCredited; detection alone is demotion, not a violation), over ${contextualEmissionCount} contextual emissions detected and ${sh?.collisions ?? 0} detected collisions${trustRead.problems.length ? `; reader problems: ${trustRead.problems.join(" | ")}` : ""}` },

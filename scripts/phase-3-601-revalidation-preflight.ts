...
void (async () => {
  ...
  if (!dec.ok) return; // unreachable: the mission already exited above; narrows the decision for the plan step
  ...
  writeJson(`${OUT}/107-current-plan.json`, {
    ...
    builtFrom: { sourceContextHash: proof.currentSourceContextHash, frozenContentHash: candidate.inventory.frozenContentHash, structuralIndex: "current (HEAD)", planner: SHARD_PLANNER_ALGORITHM_VERSION, resumeMethod: dec.record.method },
    plan: shape,
  ...
  console.log(JSON.stringify({ freeze: allId, resume: dec.record.method, passAAudit: audit.verdict, recheck: recheckOk, plan: { hash: plan.planHash, shards: shape.shards, oversized: shape.oversizedShards, maxPrimary: shape.maxPrimaryChars, maxUnits: shape.maxUnitsPerShard, midSentence: shape.midSentenceShards, tokens: shape.plannerEstimatedInputTokens, matchesExpected: shape.matchesExpectedShape }, oldAccepted: oldLoad.prior.size + oldLoadNewMission.prior.size, cost: { conservative: cost.conservativeTotalUsd, cap: cost.capUsd, balance, fits, noPaidSince }, decision: !allId ? "PHASE3_601_ENVIRONMENT_BLOCKED" : !fits ? "PHASE3_601_COST_BOUND_BEFORE_START" : !noPaidSince ? "PHASE3_601_HARNESS_DEFECT" : "CLEAR_TO_EXECUTE" }, null, 1));
})();
